...
results_runs_sdf_path = os.path.join(current_dir,"../results/Thingi10K/runs_sdf")
...
class Trainer():
        ...
        for class_idx in list(samples_dict.keys()):
            ...
            for epoch in range(start_epoch, self.train_cfg['epochs']):
                ...
                with torch.no_grad():
                    ...
                    # --- checkpoint padrão (last.pt sempre, best.pt se improved) ---
                    self.save_checkpoint(save_dir, epoch, best_loss, is_best=improved)
    ...
    def get_loaders(self,class_idx):
        ...
        g = torch.Generator().manual_seed(self.train_cfg.get("split_seed", 0))
    ...
    def save_checkpoint(self, save_dir, epoch, best_loss, is_best=False):
        ckpt = {
            "epoch": epoch,
        ...
        # scheduler é opcional
        if self.train_cfg.get("lr_scheduler", False):
            ckpt["scheduler_model"] = self.scheduler_model.state_dict()

        # opcional, mas muito útil p/ retomar igualzinho
        ckpt["rng_state"] = _get_rng_state()
    ...
    def load_checkpoint_if_exists(self, save_dir):
        last_path = os.path.join(save_dir, "last.pt")
        if not os.path.exists(last_path):
            return 0, float("inf")  # start_epoch, best_loss

        ckpt = torch.load(last_path, map_location=device)

        self.model.load_state_dict(ckpt["model"])
        self.optimizer_model.load_state_dict(ckpt["optimizer_model"])

        if self.train_cfg.get("lr_scheduler", False) and "scheduler_model" in ckpt:
            self.scheduler_model.load_state_dict(ckpt["scheduler_model"])

        _set_rng_state(ckpt.get("rng_state"))

        start_epoch = int(ckpt["epoch"]) + 1
        best_loss = float(ckpt.get("best_loss", float("inf")))
        print(f"[RESUME] Loaded checkpoint from {last_path}. Resuming at epoch {start_epoch}, best_loss={best_loss:.6f}")
        return start_epoch, best_loss

if __name__=='__main__':
   
    train_cfg_path = os.path.join(config_files_path , 'train_sdf_Thingi10K.yaml')
